...
gstreamer1.0-libav gstreamer1.0-tools v4l-utils graphviz
```
...
1. **Start the Server first** (reads configuration dynamically from `server.ini`):
   ```bash
   ./video_server
   ```
...
GST_DEBUG_DUMP_DOT_DIR=/tmp ./video_client 127.0.0.1 5000
...
- **Deterministic Delivery**: In a true automotive target environment (e.g., NXP i.MX8 or Qualcomm platforms), this configuration would leverage **SRP (Stream Reservation Protocol)** to block and guarantee network bandwidth for the safety-critical video feed.
